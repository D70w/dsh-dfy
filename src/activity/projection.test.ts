...
import type { WhaleActivityState } from './types.ts'
...
describe('whale activity projection', () => {
  const initial = (): WhaleActivityState => unit.init()
  const view = (state: ReturnType<typeof initial>) => unit.wire!.view(state)

  it('folds thinking and paired tool work without exposing event content', () => {
    let state = initial()
    state = unit.apply(state, event('turn/start', 0, { turn: 1 }))
    expect(view(state)).toEqual({ mode: 'thinking', toolKind: 'none', reaction: 'none', reactionSeq: -1 })

    state = unit.apply(state, event('tool/call', 1, {
      turn: 1, step: 1, callId: 'call-1' as never, name: 'Read', arguments: '{"private":true}',
    }))
    expect(view(state)).toEqual({ mode: 'tool', toolKind: 'read', reaction: 'none', reactionSeq: -1 })
    ...
    expect(view(state)).toEqual({ mode: 'thinking', toolKind: 'none', reaction: 'none', reactionSeq: -1 })
    expect(JSON.stringify(view(state))).not.toContain('secret')
    expect(JSON.stringify(view(state))).not.toContain('private')
  })

  it('retains only a deduplicatable result kind and causing sequence', () => {
    let state = unit.apply(initial(), event('turn/start', 3, { turn: 2 }))
    state = unit.apply(state, event('turn/end', 8, { turn: 2, reason: { kind: 'completed' } }))
    expect(view(state)).toEqual({ mode: 'idle', toolKind: 'none', reaction: 'completed', reactionSeq: 8 })
    ...
    expect(view(state)).toEqual({ mode: 'idle', toolKind: 'none', reaction: 'error', reactionSeq: 12 })
  })

  it('returns the same state reference for unrelated events', () => {
    const state = initial()
    expect(unit.apply(state, event('system/message', 0, {
      turn: 1, step: 1, message: { role: 'system', content: 'private' },
    } as never))).toBe(state)
  })
